...
export async function getProviders() { if (isDemo) return { items: [{ id: "opencode-litellm", runtime: "opencode", providerFamily: "dynamic", billingMode: "api-metered", authMode: "gateway", executionZone: "worker", enabled: true, health: "ready", scope: "all" }, { id: "codex-subscription", runtime: "codex", providerFamily: "openai", billingMode: "subscription", authMode: "vendor-browser-session", executionZone: "provider-host", enabled: false, health: "blocked", scope: "local-personal" }, { id: "claude-code-subscription", runtime: "claude-code", providerFamily: "anthropic", billingMode: "subscription-credit", authMode: "vendor-browser-session", executionZone: "provider-host", enabled: false, health: "blocked", scope: "local-personal" }] }; try { return await harnessFetch<{ items: unknown[] }>("/v1/providers"); } catch { return { items: [] }; } }
