...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang={locale}><body><WebVitalsReporter /><div className="app-shell"><aside className="sidebar"><a className="brand" href="/">AICP <small>Human Control Plane</small></a><ConsoleNavigation /><div className="muted" style={{ marginTop: "auto", color: "#8fa2b8" }}>{t("demo")}<br />No provider keys required</div></aside><div className="main"><header className="topbar"><div className="muted">Governed engineering workspace</div><div className="topbar-actions"><CommandTrigger /><LocaleSwitcher /><ThemeToggle /><span className="status status-success"><span className="status-dot" />{t("ready")}</span></div></header><main>{children}</main></div></div><CommandPalette /></body></html>;
}
