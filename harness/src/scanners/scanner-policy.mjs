export function evaluateScannerPolicy(policy, result) {
  ...
  if (result.suppressionErrors?.length) {
    return { ...result, status: "error", reason: "INVALID_SUPPRESSION_POLICY" };
  }
  ...
  const blockingFindings = result.findings.filter((finding) => finding.status !== "suppressed" && blocked.has(finding.severity));
