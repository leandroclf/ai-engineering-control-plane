...
import { assertRuntimeContract } from "../runtime/runtime-contract.mjs";
import { runRuntimeCompliance } from "../runtime/runtime-compliance.mjs";
...
export class DockerWorkerManager extends WorkerManager {
  constructor({ docker, profiles, identityService, network = "none", secretResolver = async () => null, commandPolicy = null, credentials = null, opencodeConfigSource = process.env.AICP_OPENCODE_CONFIG_SOURCE ?? null, maxActiveWorkers = Number(process.env.AICP_MAX_ACTIVE_WORKERS ?? 16), runtimeContract = null, runtimeManifest = null }) {
    super();
    ...
    this.docker = docker; this.profiles = profiles; this.identityService = identityService; this.network = network; this.secretResolver = secretResolver; this.commandPolicy = commandPolicy; this.credentials = credentials; this.opencodeConfigSource = opencodeConfigSource; this.maxActiveWorkers = maxActiveWorkers; this.runtimeContract = runtimeContract ? assertRuntimeContract(runtimeContract) : null; this.runtimeManifest = runtimeManifest; this.workers = new Map();
  }
  ...
  async create(spec) {
    ...
    if (this.runtimeContract && this.runtimeContract.provider !== "opencode" && this.runtimeContract.provider !== spec.provider) throw new Error("WORKER_RUNTIME_PROVIDER_MISMATCH");
    if (this.runtimeContract?.extensions.policy === "STRICT" && this.opencodeConfigSource) throw new Error("NATIVE_EXTENSION_CONFIG_FORBIDDEN");
    ...
    try {
      ...
      const compliance = this.runtimeContract ? await runRuntimeCompliance({ inspect: inspected, exec: (command) => this.docker.exec(containerId, command), contract: this.runtimeContract, manifest: this.runtimeManifest }) : null;
      const attestation = {
      ...
      if (!attestation.nonRoot || !attestation.readOnlyRoot || attestation.dockerSocket || attestation.providerSecrets || !attestation.capabilitiesDropped.includes("ALL") || !attestation.noNewPrivileges) throw new Error("WORKER_ATTESTATION_FAILED");
      ...
      await this.docker.remove(containerId).catch(() => undefined);
      this.identityService.revoke(spec.identityToken);
      throw error;
    }
  }
